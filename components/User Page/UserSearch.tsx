...
const UserSearch: React.FC<UserSearchProps> = ({ setFilteredUsers }) => {
  ...
  const clearSearch = () => {
    setSearchTerm('');
    setFilteredUsers(queryClient.getQueryData<UserType[]>('users'));
  };
    ...
    <div className="mb-6 mt-8">
      <div className="relative flex w-full max-w-2xl items-center">
        <div className="relative flex-grow">
          ...
          />
          <Search className="absolute left-3 top-2.5 h-5 w-5 text-gray-400 peer-focus:text-orange-500" />
          {searchTerm && (
            <button
              onClick={clearSearch}
